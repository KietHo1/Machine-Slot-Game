...
public class ResultMN : Singleton<ResultMN>
{
    ...
    private void CheckWild()
    {
        ...
        SymbolData wildSymData = GameMN.Instance.gameData.symbols.Find(x => x.type == SymbolType.WILD);
        int symIndex = GameMN.Instance.gameData.symbols.FindIndex(x => x.type == SymbolType.WILD);

        if (wildSymData != null)
        {
            WinData data = new WinData();
            data.line = -2;
            data.symbolCount = countWild;
            data.symbol = symIndex;
            data.lineReward = 0;
            winDatas.Add(data);
        }
